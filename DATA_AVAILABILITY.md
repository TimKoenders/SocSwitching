...
The baseline country scripts `01`-`31` in `code/switching/data_preparation/building_micro_data/` start from country-level `data_file` objects generated by `voteswitchR::build_data_file()`. Those objects are not one-to-one raw source files. Depending on the country and election coverage, the underlying raw files can include multiple CSES files, European Voter Project files, or national election-study files. The original files should be downloaded from the relevant providers and stored in the folder structure used by the `voteswitchR` data procurement workflow.
...
The same information is summarized in `config/required_inputs.csv`. For scripts `01`-`31`, the manifest checks for the generated `voteswitchR` country bundles, not for every original raw file used to build them. To adapt paths to a local machine, copy `config/data_paths_template.yml` to `config/data_paths.yml` and edit the folder locations. `config/data_paths.yml` is ignored by Git.
...
3. Place all original files in the local folders used by `voteswitchR` and by the direct CSES Module 6 scripts.
