...
The baseline country scripts `01`-`31` in `code/data_preparation/building_micro_data/` start from country-level `data_file` objects generated by `voteswitchR::build_data_file()` and stored under `data/micro/`. Those generated objects are not one-to-one raw source files. Depending on the country and election coverage, the underlying raw files can include multiple CSES files, European Voter Project files, or national election-study files.
