...
The repository provides code and documentation for authorized users to rebuild the analysis locally.
...
```text
data/
  micro/          # Restricted respondent-level election-study files
...
| CSES modules | Comparative respondent-level election studies for several election contexts | Download from CSES after accepting terms of use. | `data/micro/` |
...
| voteswitchR infrastructure | Mapping and vote-switching infrastructure used to harmonize or compare switching data | Install or consult `voteswitchR`; respect the original package and data terms. | local R package / `data/processed/` |
...
1. Obtain the raw files from the original providers.
2. Place them in the expected local directories.
3. Run the data-preparation scripts in order.
4. Do not commit raw or processed restricted data to Git.
