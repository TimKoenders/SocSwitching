...
#   The effect of each salience change is therefore conditional on
#   the other salience changes, but not on polarization or supply-side
#   strategy variables.
#
# Net effects:
...
make_block_formula <- function(block_predictors) {
  stats::as.formula(
    paste(
...
prepare_restricted_choice_data <- function(
  ...
  cat("Flow:", flow_label, "\n")
  cat("Predictors:", paste(block_predictors, collapse = ", "), "\n")
  ...
  df_base <- df %>%
    ...
    dplyr::mutate(
      ...
      dplyr::across(dplyr::all_of(block_predictors), as.numeric),
      gender = as.numeric(gender),
      age_group = factor(age_group, levels = c("18-34", "35-54", "55+")),
      enp_z = as.numeric(enp_z),
    ...
    dplyr::filter(
      ...
      !is.na(country_id),
      !is.na(election_id),
      !dplyr::if_any(dplyr::all_of(block_predictors), is.na),
      !is.na(gender),
      !is.na(age_group),
      !is.na(enp_z),
      !is.na(weights),
  ...
  diagnostics <- tibble::tibble(
    ...
    weighted_n_choices = sum(df_base$weights, na.rm = TRUE),
    reference_alt = reference_alt,
...
fit_restricted_choice_mixed_clogit <- function(
  ...
  cat("\n================================================\n")
  cat("Fitting joint salience-change restricted-choice-set mixed conditional logit\n")
  cat("Flow:", flow_label, "\n")
  cat("Predictors:", paste(block_predictors, collapse = ", "), "\n")
  cat("Reference alternative:", reference_alt, "\n")
  ...
  model_formula <- make_block_formula(block_predictors)
  
  tictoc::tic(paste(flow, "salience_change_block", sep = "__"))
  ...
  coef_table <- coef_table %>%
    ...
    dplyr::filter(
      stringr::str_detect(term, predictor) | !stringr::str_detect(term, paste(salience_predictors, collapse = "|"))
    )
  ...
  print(
    coef_table %>%
      dplyr::filter(stringr::str_detect(term, paste(salience_predictors, collapse = "|"))),
    n = Inf,
    width = Inf
  )
...
make_prediction_formula <- function(block_predictors) {
  stats::as.formula(
    paste(
      "~",
      paste(
...
make_prediction_matrix <- function(fit, df_long, block_predictors) {
  beta_names <- names(stats::coef(fit))
  
  X <- stats::model.matrix(
    make_prediction_formula(block_predictors),
    data = df_long
  )
...
compute_salience_ames_for_predictor <- function(model_obj, df_long, predictor, predictor_label, file_stub) {
  ...
  if (isTRUE(run_delta_method_se)) {
    compute_ame_delta_se_for_predictor(
      fit = fit,
      df_long = df_long,
      block_predictors = salience_predictors,
      predictor = predictor,
      predictor_label = predictor_label,
      file_stub = file_stub,
...
run_block_model <- function(flow, flow_label, reference_alt) {
  ...
  cat("\nAverage marginal effects of a one-standard-deviation increase in each salience change, conditional on other salience changes:\n")
  ...
  saveRDS(
    ...
    file.path(output_dir, paste0("model_salience_change_", flow, ".rds"))
  )
...
tictoc::tic("Run joint salience-change restricted-choice-set mixed conditional logit models")

all_results <- list()

all_results[["salience_change__outward"]] <- run_block_model(
  flow = "outward",
  flow_label = "Outward switching",
  reference_alt = "retention"
)

all_results[["salience_change__inward"]] <- run_block_model(
  flow = "inward",
...
shares_point <- compute_risk_set_shares(
  df_out_long = all_results[["salience_change__outward"]]$data,
  df_in_long = all_results[["salience_change__inward"]]$data
)

point_net_effects <- compute_net_effects_from_ames(
  outward_ames = all_results[["salience_change__outward"]]$ames,
  inward_ames = all_results[["salience_change__inward"]]$ames,
  shares = shares_point,
...
saveRDS(
  final_results,
  file.path(output_dir, "final_salience_change_model_results.rds")
)
...
summarise_estimation_sample <- function(result_obj) {
  ...
  outcome_var <- if (result_obj$flow == "outward") {
    "outward_sd_outcome"
  } else {
    "inward_sd_outcome"
  }
