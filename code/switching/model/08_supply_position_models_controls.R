...
#
# Specification:
#   The three supply-position variables enter jointly.
#   CPDS and Parlgov controls enter through alternative-specific
#   interactions. AMEs are computed only for the supply predictors.
#
# Net effects:
...
output_dir <- file.path(
  analysis_dir,
  "models",
  "sd_restricted_choice_set_mixed_conditional_logit_country_re_supply_position_change_controls"
)
...
if (!file.exists(path_supply_all)) {
  stop("Supply-side context file not found: ", path_supply_all)
}
...
supply_specification <- paste0(
  "joint_supply_position_change_block_",
...
df_out <- df_out %>%
  dplyr::select(-dplyr::any_of(supply_predictors)) %>%
  dplyr::left_join(
    supply_context %>%
      dplyr::select(elec_id, operationalisation, dplyr::all_of(supply_predictors)),
    by = "elec_id"
  )
...
cat("\nMissing supply-side predictor values in outward data:\n")
print(
  df_out %>%
    dplyr::summarise(
      dplyr::across(
...
cat("\nMissing supply-side predictor values in inward data:\n")
print(
  df_in %>%
    dplyr::summarise(
      dplyr::across(
...
predictor_specs <- tibble::tibble(
  ...
  predictor_label = c(
    "Change in SD social investment position",
    "Change in SD economic position",
    "Change in SD cultural position"
  ),
  file_stub = c(
    "sd_social_investment_position_change",
    "sd_economic_position_change",
    "sd_cultural_position_change"
  )
)
...
all_predictor_specs <- dplyr::bind_rows(
  predictor_specs %>%
    dplyr::mutate(predictor_type = "supply_predictor"),
  control_specs %>%
...
prepare_restricted_choice_data <- function(
  ...
  df_long <- df_base %>%
    ...
    dplyr::mutate(
      ...
      model_block = "supply_position_change_controls"
    ) %>%
    dplyr::left_join(
      alt_competitor_map,
      by = c("alt" = "alt")
...
fit_restricted_choice_mixed_clogit <- function(
  ...
  cat("\n================================================\n")
  cat("Fitting joint supply-position-change restricted-choice-set mixed conditional logit with controls\n")
  cat("Flow:", flow_label, "\n")
  cat("Supply predictors:", paste(supply_predictors, collapse = ", "), "\n")
  cat("CPDS controls:", paste(cpds_control_predictors, collapse = ", "), "\n")
  cat("Vote-share controls:", paste(vote_share_control_predictors, collapse = ", "), "\n")
  ...
  tictoc::tic(paste(flow, "supply_position_change_controls", sep = "__"))
  ...
  cat("\nCoefficient table, supply-position-change terms:\n")
  print(
    coef_table %>%
      dplyr::filter(predictor %in% supply_predictors),
    n = Inf,
    width = Inf
  )
...
compute_supply_ames_for_predictor_from_beta <- function(
    beta,
    fit,
    df_long,
    model_predictors,
  ...
  df_long %>%
    ...
    dplyr::mutate(
      ...
      file_stub = file_stub,
      component = "supply_position_change",
      .before = 1
    )
}
...
compute_ame_delta_se_for_predictor <- function(
  ...
  point <- compute_supply_ames_for_predictor_from_beta(
    beta = beta_hat,
    fit = fit,
    df_long = df_long,
    model_predictors = model_predictors,
  ...
  for (k in seq_along(beta_hat)) {
    ...
    ame_plus <- compute_supply_ames_for_predictor_from_beta(
      beta = beta_plus,
      fit = fit,
      df_long = df_long,
      model_predictors = model_predictors,
    ...
    ame_minus <- compute_supply_ames_for_predictor_from_beta(
      beta = beta_minus,
      fit = fit,
      df_long = df_long,
      model_predictors = model_predictors,
...
compute_supply_ames_for_predictor <- function(model_obj, df_long, predictor, predictor_label, file_stub) {
  ...
  if (isTRUE(run_delta_method_se)) {
    ...
    compute_supply_ames_for_predictor_from_beta(
      beta = beta_hat,
      fit = fit,
      df_long = df_long,
      model_predictors = model_predictors,
...
run_block_model <- function(flow, flow_label, reference_alt) {
  ...
  cat("\nAverage marginal effects of a one-standard-deviation increase in each supply-side position change, conditional on other supply-side position changes and controls:\n")
  ...
  saveRDS(
    ...
    file.path(output_dir, paste0("model_supply_position_change_controls_", flow, ".rds"))
  )
...
tictoc::tic("Run joint supply-position-change restricted-choice-set mixed conditional logit models with controls")

all_results <- list()

all_results[["supply_position_change_controls__outward"]] <- run_block_model(
  flow = "outward",
  flow_label = "Outward switching",
  reference_alt = "retention"
)

all_results[["supply_position_change_controls__inward"]] <- run_block_model(
  flow = "inward",
...
shares_point <- compute_risk_set_shares(
  df_out_long = all_results[["supply_position_change_controls__outward"]]$data,
  df_in_long = all_results[["supply_position_change_controls__inward"]]$data
)

point_net_effects <- compute_net_effects_from_ames(
  outward_ames = all_results[["supply_position_change_controls__outward"]]$ames,
  inward_ames = all_results[["supply_position_change_controls__inward"]]$ames,
  shares = shares_point,
...
cat("\n================================================\n")
cat("Combined supply-position-change coefficients\n")
cat("================================================\n")
print(
...
saveRDS(
  final_results,
  file.path(output_dir, "final_supply_position_change_controls_model_results.rds")
)

cat("\nControlled supply-position model outputs saved to:\n")
cat(output_dir, "\n")
