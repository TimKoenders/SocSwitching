...
input_rdata  <- "C:/Users/koend/OneDrive/Bureaublad/UVA/R_Project/VoteSwitching/VoteSwitching/data/micro/gb_data_file.RData"
output_dir   <- "C:/Users/koend/OneDrive/Bureaublad/UVA/R_Project/VoteSwitching/VoteSwitching/data/micro"

output_rdata_full <- file.path(output_dir, "gb_df_long_full.RData")
output_rdata_now  <- file.path(output_dir, "gb_df_long_valid_now.RData")
output_rdata_lag  <- file.path(output_dir, "gb_df_long_valid_lag.RData")
output_rdata_both <- file.path(output_dir, "gb_df_long_valid_both.RData")
...
add_non_alternative_before_saving <- function(df) {
  ...
  stack_limits <- df %>%
    ...
    dplyr::summarise(
      old_max_stack = max(stack, na.rm = TRUE),
  ...
  non_rows <- df %>%
    ...
    dplyr::slice(1) %>%
    dplyr::ungroup() %>%
    dplyr::left_join(stack_limits, by = "elec_id") %>%
    dplyr::mutate(
      stack = non_stack,
      alt = 0,
      peid_map = "non",
    ...
    dplyr::select(-old_max_stack, -non_stack)
  
  if ("peid" %in% names(non_rows)) {
    non_rows <- non_rows %>%
      dplyr::mutate(peid = "non")
  }
  ...
  df %>%
    ...
    dplyr::mutate(
      voted_now = dplyr::case_when(
        stack == vote ~ TRUE,
        stack == non_stack & (is.na(vote) | vote %in% invalid_vote_codes | vote > old_max_stack) ~ TRUE,
        TRUE ~ FALSE
      ),
      voted_lag = dplyr::case_when(
        stack == l_vote ~ TRUE,
        stack == non_stack & (is.na(l_vote) | l_vote %in% invalid_vote_codes | l_vote > old_max_stack) ~ TRUE,
        TRUE ~ FALSE
      ),
...
stopifnot(any(df_long_full$parfam_final == "non", na.rm = TRUE))
stopifnot(any(df_long_full$peid_map == "non", na.rm = TRUE))
...
coerce_types <- function(df) {
  char_vars <- intersect(
    c(
      "iso2c", "elec_id", "id", "peid", "peid_map",
