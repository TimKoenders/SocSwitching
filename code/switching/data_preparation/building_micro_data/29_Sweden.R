...
input_rdata <- "C:/Users/koend/OneDrive/Bureaublad/UVA/R_Project/VoteSwitching/VoteSwitching/data/micro/se_data_file.RData"
output_dir  <- "C:/Users/koend/OneDrive/Bureaublad/UVA/R_Project/VoteSwitching/VoteSwitching/data/micro"

output_rdata_full <- file.path(output_dir, paste0(tolower(country_prefix), "_df_long_full.RData"))
output_rdata_now  <- file.path(output_dir, paste0(tolower(country_prefix), "_df_long_valid_now.RData"))
output_rdata_lag  <- file.path(output_dir, paste0(tolower(country_prefix), "_df_long_valid_lag.RData"))
output_rdata_both <- file.path(output_dir, paste0(tolower(country_prefix), "_df_long_valid_both.RData"))

# ------------------------------------------------
# 5. Load previously built data_file object
# ------------------------------------------------
...
find_df_inside_data_file <- function(x) {
  ...
  if (is.null(nms)) {
    stop("data_file has no names and is not itself a data frame.")
  }
...
# ------------------------------------------------
cat("\n========================================\n")
cat("Diagnostics before cleaning\n")
cat("========================================\n")
...
# 6.1 Identify respondent-level data frame
...
# 6.3 Alternative-specific wide structure
...
vote_share_vars     <- grep("^vote_share_[0-9]+$", all_vars, value = TRUE)
vote_share_lag_vars <- grep("^vote_share_lag_[0-9]+$", all_vars, value = TRUE)
turnout_vars        <- grep("^turnout_[0-9]+$", all_vars, value = TRUE)
turnout_lag_vars    <- grep("^turnout_lag_[0-9]+$", all_vars, value = TRUE)
...
print(vote_share_vars)

cat("\nvote_share_lag_* variables:\n")
print(vote_share_lag_vars)

cat("\nturnout_* variables:\n")
print(turnout_vars)

cat("\nturnout_lag_* variables:\n")
print(turnout_lag_vars)
...
alt_nums <- c(
  ...
  as.integer(stringr::str_extract(vote_share_vars, "\\d+$")),
  as.integer(stringr::str_extract(vote_share_lag_vars, "\\d+$")),
  as.integer(stringr::str_extract(turnout_vars, "\\d+$")),
  as.integer(stringr::str_extract(turnout_lag_vars, "\\d+$"))
)
...
date_year_vars <- names(df)[
  stringr::str_detect(
    names(df),
    regex("date|year|time|field|survey|election", ignore_case = TRUE)
  )
]

cat("\nDate/year-like variables:\n")
print(date_year_vars)

if (length(date_year_vars) > 0) {
  for (v in date_year_vars) {
    cat("\nSummary of", v, ":\n")
    print(summary(df[[v]]))
  }
}
...
# 6.5 Required variables from available_data
print_header("Required variables from available_data")

available_country <- available_data %>%
  dplyr::filter(iso2c == country_prefix)

cat("\nRows in available_data for country:\n")
print(nrow(available_country))

cat("\nColumns in available_data:\n")
print(names(available_country))

var_cols <- names(available_country)[
  stringr::str_detect(
    names(available_country),
    regex("var|vote|weight|age|birth|gender|sex|educ|income", ignore_case = TRUE)
  )
]

cat("\nPotential variable-specification columns in available_data:\n")
print(var_cols)

if (length(var_cols) > 0) {
  print(
    available_country %>%
      dplyr::select(elec_id, dplyr::any_of(var_cols)) %>%
      dplyr::distinct()
  )
}

specified_vars <- available_country %>%
  dplyr::select(dplyr::any_of(var_cols)) %>%
  unlist(use.names = FALSE) %>%
  as.character()

specified_vars <- specified_vars[
  !is.na(specified_vars) &
    specified_vars != "" &
    specified_vars %in% names(df)
]

cat("\nSpecified variables found in df:\n")
print(sort(unique(specified_vars)))

if (length(specified_vars) > 0) {
  label_table <- tibble::tibble(
    variable = sort(unique(specified_vars)),
    label = purrr::map_chr(variable, ~ {
      lab <- attr(df[[.x]], "label", exact = TRUE)
      if (is.null(lab)) NA_character_ else as.character(lab)
    })
  )
  
  cat("\nVariable labels for specified variables found in df:\n")
  print(label_table, n = Inf)
}

# 6.6 Mapping structure
print_header("voteswitchR mapping structure")
...
cat("\nRows in mapping for country:\n")
print(nrow(mapping_country))

cat("\nElections in mapping:\n")
print(sort(unique(mapping_country$elec_id)))

cat("\nCounts by elec_id in mapping:\n")
print(mapping_country %>% dplyr::count(elec_id, sort = FALSE))

cat("\nDistinct parfam / parfam_harmonized combinations:\n")
mapping_country %>%
  dplyr::distinct(parfam, parfam_harmonized) %>%
  dplyr::arrange(parfam, parfam_harmonized) %>%
  tibble::as_tibble() %>%
  print(n = Inf)
...
mapping_country %>%
  ...
  dplyr::arrange(elec_id, stack) %>%
  head(100) %>%
  tibble::as_tibble() %>%
  print(n = 100)

# 6.7 Diagnostic reshape
print_header("Diagnostic reshape from wide to long")

stub_pattern <- paste0(
  "^(stack|peid|party|party_harmonized|map_vote|map_lr|",
  "vote_share|vote_share_lag|turnout|turnout_lag)_[0-9]+$"
)
...
print(long_diag %>% dplyr::count(elec_id, alt, sort = FALSE), n = Inf)

cat("\nNumber of rows with missing stack:\n")
print(sum(is.na(long_diag$stack)))

cat("\nCounts by elec_id and stack missingness:\n")
print(
  long_diag %>%
    dplyr::count(elec_id, stack_missing = is.na(stack), sort = FALSE),
  n = Inf
)

# 6.8 Remove padded alternatives before mapping
...
print(long_nopad %>% dplyr::count(elec_id, sort = FALSE), n = Inf)

# 6.9 Mapping join diagnostics
print_header("Mapping join diagnostics")
...
cat("\nMissing parfam after join:\n")
print(sum(is.na(df_mapcheck$parfam)))

cat("\nMissing parfam_harmonized after join:\n")
print(sum(is.na(df_mapcheck$parfam_harmonized)))
...
cat("\nDistinct parfam / parfam_harmonized / party_name_map combinations:\n")
print(
  df_mapcheck %>%
    dplyr::distinct(parfam, parfam_harmonized, party_name_map) %>%
    dplyr::arrange(parfam_harmonized, parfam, party_name_map),
  n = Inf
)

# 6.10 Vote-match diagnostics
print_header("Vote-match diagnostics")

if (!all(c("vote", "l_vote", "id") %in% names(df_mapcheck))) {
  stop("One or more required variables are missing: id, vote, l_vote.")
}
...
print(match_diag %>% dplyr::filter(now_matches > 1), n = 50)

cat("\nCases with lag_matches > 1:\n")
print(match_diag %>% dplyr::filter(lag_matches > 1), n = 50)

cat("\nExamples with zero now_matches:\n")
print(
  match_diag %>%
    dplyr::filter(now_matches == 0) %>%
    head(25)
)

cat("\nExamples with zero lag_matches:\n")
print(
  match_diag %>%
    dplyr::filter(lag_matches == 0) %>%
    head(25)
)

# 6.11 Vote missingness and observed vote codes
print_header("Vote missingness and observed vote codes")

cat("\nVote missingness by election:\n")
df %>%
  dplyr::distinct(elec_id, vote) %>%
  dplyr::arrange(elec_id, vote) %>%
  tibble::as_tibble() %>%
  print(n = Inf)


# 6.12 Country-specific family diagnostics
print_header("Country-specific family diagnostics")

cat("\nParties coded as parfam_harmonized == 'nat':\n")
print(
  df_mapcheck %>%
    dplyr::filter(parfam_harmonized == "nat") %>%
    dplyr::distinct(elec_id, stack, party_name_map, partyabbrev_map, parfam, parfam_harmonized) %>%
    dplyr::arrange(elec_id, stack),
  n = Inf
)

cat("\nObserved vote codes by election:\n")
df %>%
  dplyr::distinct(elec_id, vote) %>%
  dplyr::arrange(elec_id, vote) %>%
  tibble::as_tibble() %>%
...
cat("\nObserved lagged vote codes by election:\n")
df %>%
  dplyr::distinct(elec_id, l_vote) %>%
  dplyr::arrange(elec_id, l_vote) %>%
  tibble::as_tibble() %>%
  print(n = Inf)
...
df_mapcheck %>%
  dplyr::filter(parfam_harmonized == "nat") %>%
  dplyr::distinct(elec_id, stack, party_name_map, parfam, parfam_harmonized) %>%
  dplyr::arrange(elec_id, stack) %>%
  tibble::as_tibble() %>%
  print(n = Inf)
