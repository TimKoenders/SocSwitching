...
CTX_ELEC_ID_LAG   <- "PL-2019-10"
CTX_ELECTION_DATE <- as.Date("2023-10-15")
CTX_ELECTION_LAG  <- as.Date("2019-10-13")
CTX_YEAR          <- 2023L
...
output_rdata_full <- file.path(output_dir, "pl_2023_df_long_full.RData")
output_rdata_now  <- file.path(output_dir, "pl_2023_df_long_valid_now.RData")
output_rdata_lag  <- file.path(output_dir, "pl_2023_df_long_valid_lag.RData")
output_rdata_both <- file.path(output_dir, "pl_2023_df_long_valid_both.RData")
...
clean_cses_numeric <- function(x) {
  x <- num(x)
  x[x %in% c(93, 95, 96, 97, 98, 99, 9997, 9998, 9999,
             999992, 999993, 999997, 999998, 999999)] <- NA_real_
  x
}
...
party_parlgov <- tibble::tribble(
  ...
  "United Right", NA_real_,
  "Civic Coalition", NA_real_,
  "Third Way", NA_real_,
  "The Left", NA_real_,
  "Confederation Liberty and Independence", NA_real_,
  "Nonpartisan Local Government Activists", NA_real_,
  "There is One Poland", NA_real_,
  "Polish Coalition", NA_real_
)
...
party_family_map <- tibble::tribble(
  ...
  "Nonpartisan Local Government Activists", "oth",
...
stopifnot(nrow(cses_ctx) > 0)

valid_vote_t_codes   <- stats::na.omit(party_map$map_vote_t)
valid_vote_lag_codes <- stats::na.omit(party_map$map_vote_tm1)
...
df_long <- df_wide %>%
  ...
  dplyr::mutate(
    voted_now = dplyr::case_when(
      part == 1L & vote_raw %in% valid_vote_t_codes &
        !is.na(map_vote_t) & vote_raw == map_vote_t ~ TRUE,
      part == 0L & stack == non_stack ~ TRUE,
      part == 1L & (is.na(vote_raw) | !(vote_raw %in% valid_vote_t_codes)) &
        stack == non_stack ~ TRUE,
      is.na(part) & stack == non_stack ~ TRUE,
      TRUE ~ FALSE
    ),
    voted_lag = dplyr::case_when(
      l_part == 1L & l_vote_raw %in% valid_vote_lag_codes &
        !is.na(map_vote_tm1) & l_vote_raw == map_vote_tm1 ~ TRUE,
      l_part == 0L & stack == non_stack ~ TRUE,
      l_part == 1L & (is.na(l_vote_raw) | !(l_vote_raw %in% valid_vote_lag_codes)) &
        stack == non_stack ~ TRUE,
      is.na(l_part) & stack == non_stack ~ TRUE,
      TRUE ~ FALSE
    ),
