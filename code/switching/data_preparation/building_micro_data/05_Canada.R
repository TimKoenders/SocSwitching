...
input_rdata  <- "C:/Users/koend/OneDrive/Bureaublad/UVA/R_Project/VoteSwitching/VoteSwitching/data/micro/ca_data_file.RData"
output_rdata <- "C:/Users/koend/OneDrive/Bureaublad/UVA/R_Project/VoteSwitching/VoteSwitching/data/micro/ca_df_long.RData"
...
cat("\n========================================\n")
cat("MAPPING STRUCTURE (Canada)\n")
cat("========================================\n")

mappings <- voteswitchR::mappings

map_ca <- mappings %>%
  dplyr::filter(stringr::str_starts(elec_id, "CA"))

print(dim(map_ca))
print(length(unique(map_ca$elec_id)))
print(table(map_ca$elec_id))

print(unique(map_ca$parfam))
print(unique(map_ca$parfam_harmonized))
...
output_rdata_full <- file.path(output_dir, "ca_df_long_full.RData")
output_rdata_now  <- file.path(output_dir, "ca_df_long_valid_now.RData")
output_rdata_lag  <- file.path(output_dir, "ca_df_long_valid_lag.RData")
output_rdata_both <- file.path(output_dir, "ca_df_long_valid_both.RData")
