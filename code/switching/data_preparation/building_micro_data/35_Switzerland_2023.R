...
CTX_ISO2C         <- "CH"
CTX_COUNTRY       <- "Switzerland"
CTX_ELEC_ID       <- "CH-2023-10"
CTX_ELEC_ID_LAG   <- "CH-2019-10"
CTX_ELECTION_DATE <- as.Date("2023-10-22")
CTX_ELECTION_LAG  <- as.Date("2019-10-20")
CTX_YEAR          <- 2023L
...
output_rdata_full <- file.path(output_dir, "ch_2023_df_long_full.RData")
output_rdata_now  <- file.path(output_dir, "ch_2023_df_long_valid_now.RData")
output_rdata_lag  <- file.path(output_dir, "ch_2023_df_long_valid_lag.RData")
output_rdata_both <- file.path(output_dir, "ch_2023_df_long_valid_both.RData")
...
clean_cses_numeric <- function(x) {
  x <- num(x)
  x[x %in% c(93, 96, 97, 98, 99, 999992, 999993, 999997, 999998, 999999)] <- NA_real_
  x
}
...
party_family_map <- tibble::tribble(
  ~party_name, ~family,
  "Swiss People's Party",       "nat",
  "Social Democratic Party",    "soc",
  "The Liberals",               "lib",
  "The Centre",                 "chr",
  "Green Party",                "eco",
  "Green Liberal Party",        "lib",
  "Evangelical People's Party", "chr",
  "Federal Democratic Union",   "nat",
  "Swiss Party of Labour",      "com",
  "Ticino League",              "nat",
  "Geneva Citizens' Movement",  "nat",
  "Christian Social Party",     "chr"
)
