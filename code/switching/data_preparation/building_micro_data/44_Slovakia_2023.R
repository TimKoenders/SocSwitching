...
CTX_ISO2C         <- "SK"
CTX_COUNTRY       <- "Slovakia"
CTX_ELEC_ID       <- "SK-2023-09"
CTX_ELEC_ID_LAG   <- "SK-2020-02"
CTX_ELECTION_DATE <- as.Date("2023-09-30")
CTX_ELECTION_LAG  <- as.Date("2020-02-29")
CTX_YEAR          <- 2023L
...
output_rdata_full <- file.path(output_dir, "sk_2023_df_long_full.RData")
output_rdata_now  <- file.path(output_dir, "sk_2023_df_long_valid_now.RData")
output_rdata_lag  <- file.path(output_dir, "sk_2023_df_long_valid_lag.RData")
output_rdata_both <- file.path(output_dir, "sk_2023_df_long_valid_both.RData")
...
party_parlgov <- tibble::tribble(
  ...
  "Hungarian Community", NA_integer_,
  "Good Choice", 2797L,
  "Homeland", 2798L
)
...
party_family_map <- tibble::tribble(
  ~party_name, ~family,
  "Direction - Social Democracy", "soc",
  "Progressive Slovakia", "lib",
  "Voice - Social Democracy", "soc",
  "OLaNO and Friends", "con",
  "Christian Democratic Movement", "chr",
  "Freedom and Solidarity", "lib",
  "Slovak National Party", "nat",
  "Movement REPUBLIKA", "nat",
  "Hungarian Alliance", "eth",
  "Democrats", "lib",
  "We Are Family", "con",
  "Kotlebists", "nat",
  "Communist Party", "com",
