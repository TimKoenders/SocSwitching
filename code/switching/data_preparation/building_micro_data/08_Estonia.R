...
input_rdata  <- "C:/Users/koend/OneDrive/Bureaublad/UVA/R_Project/VoteSwitching/VoteSwitching/data/micro/ee_data_file.RData"
output_rdata <- "C:/Users/koend/OneDrive/Bureaublad/UVA/R_Project/VoteSwitching/VoteSwitching/data/micro/ee_df_long.RData"
...
map_ee <- mappings %>%
  dplyr::filter(stringr::str_starts(elec_id, "EE")) %>%
  dplyr::transmute(
    elec_id,
...
df_long <- df %>%
  ...
  dplyr::left_join(map_ee, by = c("elec_id", "stack")) %>%
  dplyr::mutate(
...
output_rdata_full <- file.path(output_dir, "ee_df_long_full.RData")
output_rdata_now  <- file.path(output_dir, "ee_df_long_valid_now.RData")
output_rdata_lag  <- file.path(output_dir, "ee_df_long_valid_lag.RData")
output_rdata_both <- file.path(output_dir, "ee_df_long_valid_both.RData")
