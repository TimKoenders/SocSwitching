...
CTX_ELEC_ID_LAG   <- "AT-2019-09"
CTX_ELECTION_DATE <- as.Date("2024-09-29")
CTX_ELECTION_LAG  <- as.Date("2019-09-29")
CTX_YEAR          <- 2024L
...
output_rdata_now  <- file.path(output_dir, "at_2024_df_long_valid_now.RData")
output_rdata_lag  <- file.path(output_dir, "at_2024_df_long_valid_lag.RData")
output_rdata_both <- file.path(output_dir, "at_2024_df_long_valid_both.RData")
...
party_family_map <- tibble::tribble(
  ~party_name, ~family,
  "Freedom Party of Austria",            "nat",
  "Austrian People's Party",             "chr",
  "Social Democratic Party of Austria",  "soc",
  "NEOS",                                "lib",
  "The Greens",                          "eco",
  "Communist Party of Austria",          "com",
  "JETZT - Pilz List",                   "eco"
)
...
stopifnot(
  ...
  !any(is.na(party_map$parlgov_id_1))
)
