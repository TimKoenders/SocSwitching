...
CTX_ISO2C         <- "SI"
CTX_COUNTRY       <- "Slovenia"
CTX_ELEC_ID       <- "SI-2022-04"
CTX_ELEC_ID_LAG   <- "SI-2018-06"
CTX_ELECTION_DATE <- as.Date("2022-04-24")
CTX_ELECTION_LAG  <- as.Date("2018-06-03")
CTX_YEAR          <- 2022L
...
output_rdata_now  <- file.path(output_dir, "si_2022_df_long_valid_now.RData")
output_rdata_lag  <- file.path(output_dir, "si_2022_df_long_valid_lag.RData")
output_rdata_both <- file.path(output_dir, "si_2022_df_long_valid_both.RData")
...
prune_val_labels <- function(x) {
  ...
  labelled::val_labels(x) <- labs[labs %in% vals_used]
  x
}
...
party_map_t <- tibble::tribble(
  ...
  705015L, "VESNA",
  705016L, "Democratic Pensioners Party",
  705017L, "List of Boris Popovic"
)
...
party_parlgov <- tibble::tribble(
  ...
  "VESNA", 2865L,
  "Democratic Pensioners Party", 1178L,
  "List of Boris Popovic", NA_integer_,
  "Modern Centre Party", 2618L
)

party_family_map <- tibble::tribble(
  ~party_name, ~family,
  "Freedom Movement", "lib",
  "Slovenian Democratic Party", "con",
  "New Slovenia", "chr",
  "Social Democrats", "soc",
  "The Left", "lef",
  "List of Marjan Sarec", "cen",
  "Let's Connect Slovenia", "cen",
  "Resni.ca", "rad",
  "Party of Alenka Bratusek", "lib",
  "Healthy Society Movement", "oth",
  "Good State", "cen",
  "Pirate Party", "lib",
  "Our Country", "rad",
  "Slovenian National Party", "rad",
  "VESNA", "eco",
  "Democratic Pensioners Party", "soc",
  "List of Boris Popovic", "oth",
  "Modern Centre Party", "cen"
)
